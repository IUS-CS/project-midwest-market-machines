 ...
 * historical price data to the frontend, with data from each wrapped with tags
...
public:
  ...
  inline void recordTransaction(json *transactionData) {
    ...
    if (type == "buy") {
      ...
      holdings[coin] -= quantity;
      //* Boundary case to test coin removal, not permanent, as Bounds checking
      // was supposed to be done in frontend
      if (holdings[coin] <= 0)
        holdings.erase(coin);
    }
   ...
   * Wraps JSON entries with dataType: "historical" and sends them to the
   * frontend on connection open, Final entry has last: true to signal
   * completion for frontend
   */
  inline void sendHistoricalData(ix::WebSocket &webSocket, const string &coin) {
    map<string, string> fileMap = {
        {"btcusdt", "btc-usd-max.csv"}, {"ethusdt", "eth-usd-max.csv"},
        {"adausdt", "ada-usd-max.csv"}, {"xrpusdt", "xrp-usd-max.csv"},
        {"dotusdt", "dot-usd-max.csv"}, {"uniusdt", "uni-usd-max.csv"}};
    ...
    while (getline(file, line)) {
      ...
      getline(ss, price, ',');
      // marketCap and totalVolume are discarded, but we need to read them in to
